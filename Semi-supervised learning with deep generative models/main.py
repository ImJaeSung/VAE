...
valid_loader = DataLoader(validset, 128, shuffle = True, drop_last = True)
...
#%%
"""M1 train"""
model = M1model(784, [600, 600], 50, [600, 600]).to(device)
...
for epoch in tqdm(range(epochs)):
    ...
    avg_loss = M1train(model, optimizer, train_loader, device)
    avg_valid_loss = M1eval(model, valid_loader, device)
...
torch.save(model.state_dict(), 'models/M1_state.pt')
...
model = M2model(784, [500, 500], 10, 
                784, [500, 500], 50, [500, 500], 784).to(device)
# optimizer = optim.Adagrad(model.parameters(), lr = 0.001)
optimizer = optim.Adam(model.parameters(), lr = 0.0003, betas = (0.9, 0.999))
...
for epoch in tqdm(range(epochs)):
    ...
    avg_loss = M2train(model, optimizer, train_loader_l, train_loader_ul, device)
    avg_valid_loss = M2eval(model, valid_loader_l, valid_loader_ul, device)
    print(f'[Epoch {epoch + 1}] Loss: {avg_loss:.4f}')
    print(f'[Epoch {epoch + 1}] Validation Loss: {avg_valid_loss:.4f}')
    

torch.save(model, 'models/M2.pt')
torch.save(model.state_dict(), 'models/M2_state.pt')
    # 조기 종료 및 모델 저장
    # if avg_valid_loss < best_loss:
    #     best_loss = avg_valid_loss
    #     best_model = copy.deepcopy(model)
...
#%%
"""M2 accuaracy"""
model = torch.load("models/M2.pt").to(device)
print(accuracy(model, valid_loader, device)) # 93.1891%
...
modelM1 = torch.load("models/M1.pt").to(device)
model = M2model(50, [500, 500], 10, 50, [500, 500], 50, [500, 500], 784).to(device)
optimizer = optim.Adam(model.parameters(), lr = 0.0003, betas = (0.9, 0.999))
# modelM1.eval()
# with torch.no_grad():
...
for epoch in tqdm(range(epochs)):
    ...
    avg_loss = M1_M2train(modelM1, model, optimizer, train_loader_l, train_loader_ul, device)
    avg_valid_loss = M1_M2eval(modelM1, model, valid_loader_l, valid_loader_ul, device)
    print(f'[Epoch {epoch + 1}] Loss: {avg_loss:.4f}')
    print(f'[Epoch {epoch + 1}] Validation Loss: {avg_valid_loss:.4f}')  
...
# %%
"""M1+M2 accuaracy"""
model = torch.load("models/M1_M2.pt").to(device)
modelM1 = torch.load("models/M1.pt").to(device)
...
for x, y in valid_loader:
    ...
    accuarrcies.append(torch.sum(pred == y)/z1.size(0))
